...
<script>
  ...
  function setupMonster(name, old) {
    ...
    viewport.snapZoom({
      height: ((displayMonster.height * 1.2) / window.innerHeight) * SCREEN_HEIGHT
    });
  }
  ...
  function setupCamera(camera, old) {
    switch (camera) {
      case CAMERA_STATE_MENU:
        ...
        viewport.snapZoom({
          height:
            ((displayMonster.height * 1.2) / window.innerHeight) * SCREEN_HEIGHT
        });
  ...
  function swing() {
    count += 0.003;
    const x = (Math.cos(count) * (1 + Math.random())) / 25;
    const y = (Math.sin(count) * (1 + Math.random())) / 30;
    viewport.moveCenter({
